...
func (f *FontMaker) MakeDefinitionFile(gofontname string, mappath string, exportfile string, encode string, fontmaps []FontMap, info TtfInfo) (string, error) {
	...
	str += "import (\n"
	str += "	\"github.com/cdvelop/tinypdf\"\n"
	str += ")\n"
	str += "type " + gofontname + " struct {\n"
	str += "\tfamily string\n"
	str += "\tfonttype string\n"
	str += "\tname string\n"
	str += "\tdesc  []tinypdf.FontDescItem\n"
	str += "\tup int\n"
	str += "\tut int\n"
	str += "\tcw tinypdf.FontCw\n"
	str += "\tenc string\n"
	str += "\tdiff string\n"
	str += "}\n"
	...
	str += "\treturn me.name\n"
	str += "}	\n"
	str += "func (me * " + gofontname + ")GetDesc() []tinypdf.FontDescItem{\n"
	str += "\treturn me.desc\n"
	str += "}\n"
	...
	str += "\treturn me.ut\n"
	str += "}\n"
	str += "func (me * " + gofontname + ")GetCw() tinypdf.FontCw{\n"
	str += "\treturn me.cw\n"
	str += "}\n"
...
func (f *FontMaker) MakeFontDescriptor(info TtfInfo) (string, error) {
	...
	fd := ""
	fd = "\tme.desc = make([]tinypdf.FontDescItem,8)\n"
	...
	fd += fmt.Sprintf("\tme.desc[0] =  tinypdf.FontDescItem{ Key:\"Ascent\",Val : \"%d\" }\n", ascender)
	...
	fd += fmt.Sprintf("\tme.desc[1] =  tinypdf.FontDescItem{ Key: \"Descent\", Val : \"%d\" }\n", descender)

	// CapHeight
	capHeight, err := info.GetInt64("CapHeight")
	if err == nil {
		fd += fmt.Sprintf("\tme.desc[2] =  tinypdf.FontDescItem{ Key:\"CapHeight\", Val :  \"%d\" }\n", capHeight)
	...
	fd += fmt.Sprintf("\tme.desc[3] =  tinypdf.FontDescItem{ Key: \"Flags\", Val :  \"%d\" }\n", flags)
	...
	fd += fmt.Sprintf("\tme.desc[4] =  tinypdf.FontDescItem{ Key:\"FontBBox\", Val :  \"[%d %d %d %d]\" }\n", fbb[0], fbb[1], fbb[2], fbb[3])

	// ItalicAngle
	fd += fmt.Sprintf("\tme.desc[5] =  tinypdf.FontDescItem{ Key:\"ItalicAngle\", Val :  \"%d\" }\n", italicAngle)
	...
	fd += fmt.Sprintf("\tme.desc[6] =  tinypdf.FontDescItem{ Key:\"StemV\", Val :  \"%d\" }\n ", stemv)
	...
	fd += fmt.Sprintf("\tme.desc[7] =  tinypdf.FontDescItem{ Key:\"MissingWidth\", Val :  \"%d\" } \n ", missingWidth)
	return fd, nil
}
...
func (f *FontMaker) MakeWidthArray(widths map[int]int) (string, error) {
	...
	for c := 0; c <= 255; c++ {
		...
		if chr == "\"" {
			...
			str += "tinypdf.ToByte(\"" + chr + "\")"
		} else {
			str += fmt.Sprintf("tinypdf.Chr(%d)", c)
		}
		str += fmt.Sprintf("]=%d\n", widths[c])
	}
	return str, nil
