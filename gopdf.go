...
var ErrEmptyString = Err("empty string")

var ErrMissingFontFamily = Err("font family not found")

var ErrUndefinedCacheContentImage = Err("cacheContentImage is undefined")

var ErrInvalidRectangleCoordinates = Err("Invalid coordinates for the rectangle")

var ErrInvalidRectangleRadius = Err("Radius length cannot exceed rectangle height or width")
...
func convertNumericToFloat64(size interface{}) (fontSize float64, err error) {
	...
	case uint8:
		return float64(size), nil
	default:
		return 0.0, Err("fontSize must be of type (u)int* or float*, not", size)
	}
}
...
func (gp *GoPdf) ImportPagesFromSource(source interface{}, box string) error {
	...
	case *io.ReadSeeker:
		...
		gp.fpdi.SetSourceStream(v)
	default:
		return Err("source type not supported")
	}
	...
	for i := 0; i < pages; i++ {
		...
		if !ok {
			return Err("can not get page size")
		}
