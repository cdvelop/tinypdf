...
// tinypdf and its various contributed packages located beneath the contrib
// directory.
package example
...
func init() {
	setRoot()
	tinypdf.SetDefaultCompression(false)
	tinypdf.SetDefaultCatalogSort(true)
	tinypdf.SetDefaultCreationDate(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC))
	tinypdf.SetDefaultModificationDate(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC))
}
...
func setRoot() {
	...
	if err == nil {
		...
		for j := len(list) - 1; j >= 0 && list[j] != "tinypdf"; j-- {
