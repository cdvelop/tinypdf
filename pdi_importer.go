...
func newImporter(log func(a ...any)) *importer {
	importer := &importer{
		log: log,
	}
	importer.init()
...
func (this *importer) init() {
	...
	this.writer, _ = newPdfWriter("", this.log)
...
func (this *importer) SetSourceFile(f string) {
	...
	if _, ok := this.readers[this.sourceFile]; !ok {
		reader, err := newPdfReader(this.sourceFile, this.log)
		if err != nil {
			panic(err)
		}
	...
	if _, ok := this.writers[this.sourceFile]; !ok {
		writer, err := newPdfWriter("", this.log)
		if err != nil {
			panic(err)
		}
...
func (this *importer) SetSourceStream(rs *io.ReadSeeker) {
	...
	if _, ok := this.readers[this.sourceFile]; !ok {
		reader, err := newPdfReaderFromStream(this.sourceFile, *rs, this.log)
		if err != nil {
			panic(err)
		}
		this.readers[this.sourceFile] = reader
