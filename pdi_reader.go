...
func newPdfReader(filename string, log func(...any)) (*pdfReader, error) {
	...
	if err != nil {
		...
		return nil, Err("file open failed")
	}
	info, err := f.Stat()
	if err != nil {
		log("Failed to get file info:", err)
		return nil, Err("file stat failed")
	}
	...
	if err = parser.read(); err != nil {
		...
		return nil, Err("PDF read failed")
	}
	return parser, nil
}
...
func (this *pdfReader) init() error {
	...
	if err != nil {
		...
		return Err("PDF read failed during init")
	}
	return nil
}
...
func (this *pdfReader) skipComments(r *bufio.Reader) error {
	...
	for {
		...
		if err != nil {
			...
			return Err("comment skip failed")
		}
...
func (this *pdfReader) skipWhitespace(r *bufio.Reader) error {
	...
	for {
		...
		if err != nil {
			...
			return Err("whitespace skip failed")
		}
...
func (this *pdfReader) readToken(r *bufio.Reader) (string, error) {
	...
	if err != nil {
		return "", Err(err, "Failed to skip whitespace")
	}
	...
	if err != nil {
		...
		return "", Err(err, "Failed to read byte")
	}
	...
	case '%':
		...
		if err != nil {
			return "", Err(err, "Failed to skip comments")
		}
...
func (this *pdfReader) readValue(r *bufio.Reader, t string) (*PdfValue, error) {
	...
	case "<":
		...
		for {
			b, err = r.ReadByte()
			if err != nil {
				return nil, Err(err, "Failed to read byte")
			}
			if b != '>' {
				s += string(b)
			} else {
	...
	case "<<":
		...
		for {
			...
			if err != nil {
				return nil, Err(err, "Failed to read token")
			}
			if key == "" {
				return nil, Err("Token is empty")
			}
			...
			if err != nil {
				return nil, Err(err, "Failed to read value for token: "+newKey)
			}
	...
	case "[":
		...
		for {
			...
			if err != nil {
				return nil, Err(err, "Failed to read value for token: "+key)
			}
	...
	case "stream":
		return nil, Err("Stream not implemented")
...
func (this *pdfReader) resolveCompressedObject(objSpec *PdfValue) (*PdfValue, error) {
	...
	if _, ok := this.xrefStream[objSpec.Id]; !ok {
		return nil, Err(fmt.Sprintf("Could not find object ID %d in xref stream or xref table.", objSpec.Id))
	}
	...
	if err != nil {
		return nil, Err(err, "Failed to resolve compressed object")
	}
	...
	if _, ok := compressedObj.Value.Dictionary["/Type"]; ok {
		if compressedObj.Value.Dictionary["/Type"].Token != "/ObjStm" {
			return nil, Err("Expected compressed object type to be /ObjStm")
		}
	} else {
		return nil, Err("Could not determine compressed object type.")
	}

	// Get number of sub-objects in compressed object
	n := compressedObj.Value.Dictionary["/N"].Int
	if n <= 0 {
		return nil, Err("No sub objects in compressed object")
	}
	...
	if _, ok := compressedObj.Value.Dictionary["/Filter"]; ok {
		...
		if filter != "/FlateDecode" {
			return nil, Err("Unsupported filter - expected /FlateDecode, got: " + filter)
		}
	}
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			return nil, Err(err, "Failed to convert token into integer: "+token)
		}
	...
	if err != nil {
		return nil, Err(err, "Failed to read value for token: "+token)
	}
...
func (this *pdfReader) resolveObject(objSpec *PdfValue) (*PdfValue, error) {
	...
	if objSpec.Type == pdf_type_objref {
		...
		if err != nil {
			return nil, Err(err, "Failed to get current position of file")
		}

		// Reposition the file pointer and load the object header
		_, err = this.f.Seek(int64(offset), 0)
		if err != nil {
			return nil, Err(err, "Failed to set position of file")
		}
		...
		if obj.Type != pdf_type_objdec {
			return nil, Err(fmt.Sprintf("Expected type to be pdf_type_objdec, got: %d", obj.Type))
		}

		if obj.Id != objSpec.Id {
			return nil, Err(fmt.Sprintf("Object ID (%d) does not match ObjSpec ID (%d)", obj.Id, objSpec.Id))
		}

		if obj.Gen != objSpec.Gen {
			return nil, Err("Object Gen does not match ObjSpec Gen")
		}
		...
		if token == "stream" {
			...
			err = this.skipWhitespace(r)
			if err != nil {
				return nil, Err(err, "Failed to skip whitespace")
			}
			...
			if lengthDict.Type == pdf_type_objref {
				...
				if err != nil {
					return nil, Err(err, "Failed to resolve length object of stream")
				}
			...
			if err != nil {
				return nil, Err(err, "Failed to read bytes from buffer")
			}
			...
			if token != "endstream" {
				return nil, Err("Expected next token to be: endstream, got: " + token)
			}
		...
		if token != "endobj" {
			return nil, Err("Expected next token to be: endobj, got: " + token)
		}
...
func (this *pdfReader) findXref() error {
	...
	if err != nil {
		return Err(err, "Failed to set position of file")
	}
	...
	for {
		...
		token, err := this.readToken(r)
		if err != nil {
			return Err(err, "Failed to read token")
		}
		...
		if token == "startxref" {
			...
			if err != nil {
				return Err(err, "Failed to find startxref token")
			}

			// Convert line (string) into int
			result, err = strconv.Atoi(token)
			if err != nil {
				return Err(err, "Failed to convert xref position into integer: "+token)
			}
...
func (this *pdfReader) readXref() error {
	...
	if t != "xref" {
		...
		if err != nil {
			return Err(err, "Failed to read XRef stream")
		}
		...
		if v.Type == pdf_type_objdec {
			...
			v, err := this.readValue(r, t)
			if err != nil {
				return Err(err, "Failed to read value for token: "+t)
			}
				...
				if v.Dictionary["/Type"].Token == "/XRef" {
					...
					if _, ok := v.Dictionary["/DecodeParms"]; ok {
						...
						if columns > 4 || predictor > 12 {
							return Err("Unsupported /DecodeParms - only tested with /Columns <= 4 and /Predictor <= 12")
						}
						paethDecode = true
					}
					...
					/*
						...
						if v.Dictionary["/W"].Array[0].Int != 1 || v.Dictionary["/W"].Array[1].Int > 4 || v.Dictionary["/W"].Array[2].Int != 1 {
							return Err(fmt.Sprintf("Unsupported field sizes in cross-reference stream dictionary: /W [%d %d %d]",
					...
					*/
					...
					if _, ok := v.Dictionary["/Index"]; ok {
						if len(v.Dictionary["/Index"].Array) < 2 {
							return Err(err, "Index array does not contain 2 elements")
						}
					...
					if _, ok := v.Dictionary["/Root"]; ok {
						...
						//return Err("Did not set root object")
					}
					...
					if lengthDict.Type == pdf_type_objref {
						...
						if err != nil {
							return Err(err, "Failed to resolve length object of stream")
						}
					...
					if t != "stream" {
						return Err("Expected next token to be: stream, got: " + t)
					}
					...
					if err != nil {
						return Err(err, "Failed to read bytes from buffer")
					}
					...
					if t != "endstream" {
						return Err("Expected next token to be: endstream, got: " + t)
					}
					...
					if t != "endobj" {
						return Err("Expected next token to be: endobj, got: " + t)
					}
					...
					if err != nil {
						return Err(err, "zlib.NewReader error")
					}
					defer z.Close()

					p, err := io.ReadAll(z)
					if err != nil {
						return Err(err, "ioutil.ReadAll error")
					}
					...
					for {
						...
						if err != nil {
							if err == io.EOF {
								...
								return Err(err, "io.ReadFull error")
							}
						}
					...
					if prevXref > 0 {
						...
						if xrefErr != nil {
							return Err(xrefErr, "Failed to read prev xref")
						}
					}
				}
			}
		...
		return Err("Expected xref to start with 'xref'.  Got: " + t)
	}
	...
	for {
		...
		if err != nil {
			return Err(err, "Failed to convert start object to integer: "+t)
		}
		...
		if err != nil {
			return Err(err, "Failed to convert num object to integer: "+t)
		}
		...
		for i := startObject; i < startObject+numObject; i++ {
			...
			if err != nil {
				return Err(err, "Failed to convert object position to integer: "+t)
			}
			...
			if err != nil {
				return Err(err, "Failed to convert object generation to integer: "+t)
			}
			...
			if objStatus != "f" && objStatus != "n" {
				return Err("Expected objStatus to be 'n' or 'f', got: " + objStatus)
			}
...
func (this *pdfReader) readRoot() error {
	...
	if err != nil {
		return Err(err, "Failed to resolve root object")
	}
...
func (this *pdfReader) readKids(kids *PdfValue, r int) error {
	...
	for i := 0; i < len(kids.Array); i++ {
		...
		if err != nil {
			return Err(err, "Failed to resolve page/pages object")
		}
		...
		if objType == "/Page" {
			...
			if err != nil {
				return Err(err, "Failed to resolve kids")
			}

			// Recurse into page tree
			err = this.readKids(subKids, r+1)
			if err != nil {
				return Err(err, "Failed to read kids")
			}
		} else {
			return Err(err, fmt.Sprintf("Unknown object type '%s'.  Expected: /Pages or /Page", objType))
		}
	}
...
func (this *pdfReader) getPageResources(pageno int) (*PdfValue, error) {
	...
	if len(this.pages) < pageno {
		return nil, Err(fmt.Sprintf("Page %d does not exist!!", pageno))
	}

	// Resolve page object
	page, err := this.resolveObject(this.pages[pageno-1])
	if err != nil {
		return nil, Err(err, "Failed to resolve page object")
	}
	...
	if _, ok := page.Value.Dictionary["/Resources"]; ok {
		...
		if err != nil {
			return nil, Err(err, "Failed to resolve resources object")
		}
		...
		if _, ok := page.Value.Dictionary["/Parent"]; ok {
			...
			if err != nil {
				return nil, Err(err, "Failed to resolve parent object")
			}
...
func (this *pdfReader) getPageContent(objSpec *PdfValue) ([]*PdfValue, error) {
	...
	if objSpec.Type == pdf_type_objref {
		...
		if err != nil {
			return nil, Err(err, "Failed to resolve object")
		}
		...
		for i := 0; i < len(objSpec.Array); i++ {
			...
			if err != nil {
				return nil, Err(err, "Failed to get page content")
			}
...
func (this *pdfReader) getContent(pageno int) (string, error) {
	...
	// Check to make sure page exists in pages slice
	if len(this.pages) < pageno {
		return "", Err(fmt.Sprintf("Page %d does not exist.", pageno))
	}
	...
	if _, ok := page.Value.Dictionary["/Contents"]; ok {
		...
		if err != nil {
			return "", Err(err, "Failed to get page content")
		}
		...
		for i := 0; i < len(contents); i++ {
			...
			if err != nil {
				return "", Err(err, "Failed to rebuild content stream")
			}
...
func (this *pdfReader) rebuildContentStream(content *PdfValue) ([]byte, error) {
	...
	for i := 0; i < len(filters); i++ {
		...
		case "/FlateDecode":
			...
			stream = out.Bytes()
		default:
			return nil, Err("Unspported filter: " + filters[i].Token)
		}
	}
...
func (this *pdfReader) getAllPageBoxes(k float64) (map[int]map[string]map[string]float64, error) {
	...
	for i := 1; i <= len(this.pages); i++ {
		...
		if result[i] == nil {
			return nil, Err(err, "Unable to get page box")
		}
	}
...
func (this *pdfReader) getPageBoxes(pageno int, k float64) (map[string]map[string]float64, error) {
	...
	page, err := this.resolveObject(this.pages[pageno-1])
	if err != nil {
		return nil, Err("Failed to resolve page object")
	}
	...
	for i := 0; i < len(this.availableBoxes); i++ {
		...
		if err != nil {
			return nil, Err("Failed to get page box")
		}
...
func (this *pdfReader) getPageBox(page *PdfValue, box_index string, k float64) (map[string]float64, error) {
	...
	if _, ok := page.Value.Dictionary[box_index]; ok {
		...
		if box.Type == pdf_type_objref {
			...
			if err != nil {
				return nil, Err("Failed to resolve object")
			}
			box = tmpBox.Value
		}
...
func (this *pdfReader) _getPageRotation(page *PdfValue) (*PdfValue, error) {
	...
	if _, ok := page.Value.Dictionary["/Rotate"]; ok {
		...
		if err != nil {
			return nil, Err("Failed to resolve rotate object")
		}
		...
		if _, ok := page.Value.Dictionary["/Parent"]; ok {
			...
			if err != nil {
				return nil, Err(err, "Failed to get page rotation for parent")
			}
...
func (this *pdfReader) read() error {
	...
	if !this.alreadyRead {
		...
		err = this.findXref()
		if err != nil {
			return Err(err, "Failed to find xref position")
		}

		// Parse xref table
		err = this.readXref()
		if err != nil {
			return Err(err, "Failed to read xref table")
		}

		// Read catalog
		err = this.readRoot()
		if err != nil {
			return Err(err, "Failed to read root")
		}

		// Read pages
		err = this.readPages()
		if err != nil {
			return Err(err, "Failed to to read pages")
		}
