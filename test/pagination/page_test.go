...
func TestLineBreak(t *testing.T) {
	...
	var breakOptionTests = []*tinypdf.BreakOption{
		&tinypdf.DefaultBreakOption,
		{
			Mode:           tinypdf.BreakModeIndicatorSensitive,
			BreakIndicator: ' ',
		},
	}

	y := (tinypdf.PageSizeA4.H/2 + 100.0*float64(len(breakOptionTests))) / 2
	linebreakText := strings.Repeat("MultiCell* methods don't respect linebreaking rules.", 2)
	for i, opt := range breakOptionTests {
		pdf.SetXY(tinypdf.PageSizeA4.W/2-w/2, y+100.0*float64(i))
		err = pdf.MultiCellWithOption(&tinypdf.Rect{
			W: w,
			H: 1000,
		}, linebreakText, tinypdf.CellOption{
			BreakOption: opt,
		})
		if err != nil {
			log.Fatalln(err)
