...
func TestFpdfImplementPdf(t *testing.T) {
	...
	var _ tinypdf.Pdf = (*tinypdf.Fpdf)(nil)
...
// tinypdf library. If an error occurs at some point during the construction of
...
func ExampleFpdf_SplitLines_tables() {
	...
	pdf := tinypdf.New("P", "mm", "A4", "") // 210 x 297
...
func ExampleFpdf_AddFont() {
	pdf := tinypdf.New("P", "mm", "A4", example.FontDir())
...
func ExampleFpdf_SetKeywords() {
	...
	err = tinypdf.MakeFont(example.FontFile("CalligrapherRegular.pfb"),
...
func ExampleFpdf_PageSize() {
	pdf := tinypdf.NewCustom(&tinypdf.InitType{
		UnitStr:    "in",
		Size:       tinypdf.SizeType{Wd: 6, Ht: 6},
		FontDirStr: example.FontDir(),
	})
	pdf.SetMargins(0.5, 1, 0.5)
	pdf.SetFont("Times", "", 14)
	pdf.AddPageFormat("L", tinypdf.SizeType{Wd: 3, Ht: 12})
	pdf.SetXY(0.5, 1.5)
	...
	pdf.AddPageFormat("P", tinypdf.SizeType{Wd: 3, Ht: 12})
	pdf.SetXY(0.5, 6)
...
func ExampleFpdf_SplitLines() {
	...
	pdf := tinypdf.New("P", "mm", "A4", "") // A4 210.0 x 297.0
...
func ExampleFpdf_SVGBasicWrite() {
	...
	html := pdf.HTMLBasicNew()
	html.Write(lineHt, htmlStr)
	sig, err = tinypdf.SVGBasicFileParse(example.ImageFile(sigFileStr))
	if err == nil {
		scale := 100 / sig.Wd
		scaleY := 30 / sig.Ht
		if scale > scaleY {
...
func ExampleFpdf_CellFormat_align() {
	...
	var formatRect = func(pdf *tinypdf.Fpdf, recList []recType) {
		linkStr := ""
...
func ExampleFpdf_CellFormat_codepageescape() {
	...
	pdf.AddPage()
	htmlStr := `Until tinypdf supports UTF-8 encoded source text, source text needs ` +
...
func ExampleFpdf_CellFormat_codepage() {
	pdf := tinypdf.New("P", "mm", "A4", example.FontDir()) // A4 210.0 x 297.0
